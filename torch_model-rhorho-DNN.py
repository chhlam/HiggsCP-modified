...
mc_train_idx=np.random.choice(np.arange(points[particle_idx].train.x.shape[0]),
...
mc_valid_idx=np.random.choice(np.arange(points[particle_idx].valid.x.shape[0]),
...
mc_test_idx=np.random.choice(np.arange(points[particle_idx].test.x.shape[0]),
...
with torch.no_grad():
    outputs=[]
    for i in range(0, 400000,batch_size):
...
ax.savefig('BackgroundResult.pdf')