...
def read_raw_all(kind, args):
    ...
    for letter in ["a", "b", "c", "d", "e", "f", "g", "h", "i", "k"][:args.DATASETS]:
        name = os.path.join(data_path, 
            "pythia.Z_115_135.rhorho.1M.%s.outTUPLE_labFrame" % (letter))
        print(letter, name)
...
if __name__ == "__main__":
    ...
    np.save(os.path.join(data_path, "Z_115_135.rhorho_raw.data.npy"), data_a)
    np.save(os.path.join(data_path, "Z_115_135.rhorho_raw.w_a.npy"), weights_a)
    np.save(os.path.join(data_path, "Z_115_135.rhorho_raw.perm.npy"), perm)